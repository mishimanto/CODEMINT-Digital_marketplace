    ...
    <ul class="header_menu d-flex flex-wrap">
        ...
        @if ($setting->commission_type == 'subscription')
            ...
            @if ($module_status->Subscription)
                <li><a class="{{ Route::is('provider.subscription-plan-user') ? 'active' : '' }}" href="{{ route('provider.subscription-plan-user') }}"><i class="far fa-usd-circle"></i> {{__('user.Subscription')}} </a></li>
            @endif
        @else
