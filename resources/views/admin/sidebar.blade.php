...
<div class="main-sidebar">
    ...
    @if ($setting->commission_type == 'subscription')
        ...
        <li class="nav-item dropdown {{ Route::is('admin.subscription-plan.*') || Route::is('admin.purchase-history') || Route::is('admin.assign-plan') || Route::is('admin.purchase-history-show') || Route::is('admin.pending-plan-payment') ? 'active' : '' }}">
            <a href="#" class="nav-link has-dropdown" data-toggle="dropdown"><i class="fas fa-list"></i> <span>{{__('admin.Subscription')}}</span>
    ...
    @endif
        ...
        <li class="{{ Route::is('admin.payment-addon') ? 'active' : '' }}"><a class="nav-link" href="{{ route('admin.payment-addon') }}"><i class="fas fa-dollar-sign"></i> {{__('admin.Gateway')}}<small class="badge badge-danger addon_text ml-3">{{__('addon')}}</small></a></li>
