...
// https://s3.ap-southeast-1.wasabisys.com/quomodosoft-envato/logo-2024-06-12-01-02-37-1033%202.png



// https://quomodosoft-envato.s3.wasabisys.com



// https://sin1.contabostorage.com/uploads/custom-images/file-name-17289038121423.png




// $url = 'https://sin1.contabostorage.com/0a439b73d6e5457093b0cc9e33c16355:renderrush/uploads/custom-images/file-name-17289038121423.png';
...
Route::get('/test-s3-connection', function () {
    try {
        ...
        $contents = Storage::disk('contabo')->allFiles();
...
Route::group(['middleware' => ['maintainance']], function () {
    ...
    Route::get('/pay-with-grabpay', [PaymentController::class, 'pay_with_grabpay'])->name('pay-with-grabpay');

    Route::get('/pay-with-gcash', [PaymentController::class, 'pay_with_gcash'])->name('pay-with-gcash');

    Route::post('/pay-with-paymongo', [PaymentController::class, 'pay_with_paymongo'])->name('pay-with-paymongo');

    Route::post('/pay-with-iyzico', [PaymentController::class, 'pay_with_iyzico'])->name('pay-with-iyzico');

    Route::post('/pay-with-mercadopago', [PaymentController::class, 'pay_with_mercadopago'])->name('pay-with-mercadopago');
    ...
    Route::get('secure-download-script/{id}', [UserProfileController::class, 'download_script'])->name('download-script');
    Route::get('secure-download-variant-item/{id}', [UserProfileController::class, 'download_variant'])->name('download-variant-item');

    Route::get('personal-download-script/{id}', [UserProfileController::class, 'personal_download_script'])->name('personal-download-script');
    Route::get('personal-download-variant-item/{id}', [UserProfileController::class, 'personal_download_variant'])->name('personal-download-variant-item');
    ...
    // routes/web.php
    Route::get('/withdraw-report/{id}/download', [App\Http\Controllers\User\WithdrawController::class, 'downloadReport'])
        ->name('user.withdraw.report.download')
        ->middleware('auth'); // যদি লগইন করা থাকতে হয়
...
Route::group(['as'=> 'admin.', 'prefix' => 'admin'],function (){
    ...
    Route::get('default-placeholder', [ContentController::class, 'defaultPlaceholder'])->name('default-placeholder');
    Route::put('update-default-placeholder', [ContentController::class, 'updateDefaultPlaceholder'])->name('update-default-placeholder');
...
Route::prefix('admin')->name('admin.')->middleware(['auth:admin'])->group(function () {
    ...
    // Sales & Charges Reports (overview + filter)
    Route::get('/reports/sales', [ReportController::class, 'index'])->name('reports.sales');

    // Ajax/Data export (optional)
    Route::get('/reports/sales/data', [ReportController::class, 'data'])->name('reports.sales.data');
