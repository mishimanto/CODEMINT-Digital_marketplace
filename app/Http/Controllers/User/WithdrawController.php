...
class WithdrawController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($request->withdraw_amount >= $method->min_amount && $request->withdraw_amount <= $method->max_amount){
            ...
            $withdraw_charge_amount = ($method->withdraw_charge / 100) * $withdraw_request;

            $widthdraw = new ProviderWithdraw();
            $widthdraw->user_id = $author->id;
            $widthdraw->method = $method->name;
            $widthdraw->total_amount = $withdraw_request;
            $widthdraw->charged_amount = $withdraw_charge_amount;
            $widthdraw->withdraw_amount = $withdraw_request - $withdraw_charge_amount;
    ...
    {
        $withdraw = ProviderWithdraw::where('id', $id)
            ->where('user_id', auth()->id()) // শুধুমাত্র নিজের ডাটা
            ->firstOrFail();

        if (!$withdraw->report_path || !Storage::disk('public')->exists($withdraw->report_path)) {
            return back()->with('error', 'Report not found.');
        }

        return Storage::disk('public')->download($withdraw->report_path);
    }
}
